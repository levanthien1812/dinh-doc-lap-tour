...
<div className={`${style.flex} ${style.container} ${style.w_100}`}>
  <div className={`${style.grid} ${style.col_5} py-8 px-8 ${style.mt_10}`}>
    <h1 className={`${style.dark} ${style.semi_b} ${style.fs_35}`}>DI TÍCH LỊCH SỬ</h1>
    <h2 className={`${style.yellow} ${style.semi_b} ${style.fs_60}`}>DINH ĐỘC LẬP</h2>
    <h1 className={`${style.dark} ${style.semi_b} ${style.fs_35}`}>THE INDEPENDENCE PALACE</h1>
    <div className={`${style.flex} ${style.justify_center}`}>
      <img src={dinhDocLapImg} alt="" width={500} />
    </div>
  </div>
  <div className={`${style.grid} ${style.col_7} py-8 px-8 ${style.info_card}`}>
    <ul className={`${style.bg_blue} py-8 px-8`}>
      <li>Mã đặt vé: {formData.id}</li>
      <li>Ngày đặt: <span>{formData.bookingDate?.toLocaleDateString('vi-VN')}</span></li>
      ...
      <li>Số lượng mỗi loại vé: <span>Người lớn - {formData.venguoilon}, Trẻ em - {formData.vetreem}</span></li>
      <li>Tổng tiền từng loại vé: <span>Người lớn - {formData.nguoilonPrice}đ, Trẻ em - {formData.treemPrice}đ</span></li>
      <li>Tổng số lượng vé: <span>{formData.venguoilon + formData.vetreem}</span></li>
      <li>Tổng tiền: <span>{(formData.nguoilonPrice + formData.treemPrice).toLocaleString()}đ</span></li>
      <li>Trạng thái đặt vé: <span className={`${style.green}`}>{formData.status === 0 ? "Đã duyệt" : "Chờ duyệt"}</span></li>
    </ul>
  </div>
</div>
  );
